...
printf "geneticTime = ["
for (( i=1; i<=$rep; i++ )); do
./gol $width $height $iterations $modeGPUGen $gpuBlocks $nThreads 0 0
if [ $i -lt $rep ]; then
printf ","
fi
done
...
printf "gpuBestTime = ["
for (( i=1; i<=$rep; i++ )); do
./gol $width $height $iterations $modeGPUTile $gpuBlocks $nThreads 15250 100
if [ $i -lt $rep ]; then
printf ","
fi
done
...
printf "cpuTime = ["
for (( i=1; i<=$rep; i++ )); do
./gol $width $height $iterations $modeCPU $gpuBlocks $nThreads 0 0
if [ $i -lt $rep ]; then
printf ","
fi
done
...
for (( i=1; i<=$rep; i++ )); do
./gol $width $height $iterations $modeGPUTile $gpuBlocks $nThreads 15250 $dt
if [ $i -lt $rep ]; then
printf ","
fi
done
