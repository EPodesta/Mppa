...
printf "gpuBestTime = ["
for (( i=1; i<=$rep; i++ )); do
./cloudsim $width $height $iterations 20 -3 5.0 700.0 0.001 $modeGPUTile $gpuBlocks $nThreads 15100 100
if [ $i -lt $rep ]; then
printf ","
fi
done
...
for (( i=1; i<=$rep; i++ )); do
./cloudsim $width $height $iterations 20 -3 5.0 700.0 0.001 $modeGPUTile $gpuBlocks $nThreads 15100 $dt
if [ $i -lt $rep ]; then
printf ","
fi
done
...
printf "tilingTimeX2 = {}\n"
for dh in 15100 10000 7500 5000 2500 1000 500 250; do
