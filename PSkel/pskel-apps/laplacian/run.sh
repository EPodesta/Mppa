...
printf "gpuBestTime = ["
for (( i=1; i<=$rep; i++ )); do
./laplacian $width $height $depth $iterations $modeGPUTile $gpuBlocks $nThreads 20420 100
if [ $i -lt $rep ]; then
printf ","
fi
done
...
for (( i=1; i<=$rep; i++ )); do
./laplacian $width $height $depth $iterations $modeGPUTile $gpuBlocks $nThreads 20420 $dt
if [ $i -lt $rep ]; then
printf ","
fi
done
