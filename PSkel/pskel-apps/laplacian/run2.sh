...
printf "geneticTime = ["
for (( i=1; i<=$rep; i++ )); do
./laplacian $width $height $depth $iterations $modeGPUGen $gpuBlocks $nThreads 0 0
if [ "$i" -lt "$rep" ]; then
printf ","
fi
done
...
printf "gpuBestTime = ["
for (( i=1; i<=$rep; i++ )); do
./laplacian $width $height $depth $iterations $modeGPUTile $gpuBlocks $nThreads 15250 100
if [ $i -lt $rep ]; then
printf ","
fi
done
...
printf "cpuTime = ["
for (( i=1; i<=$rep; i++ )); do
./laplacian $width $height $depth $iterations $modeCPU $gpuBlocks $nThreads 0 0
if [ $i -lt $rep ]; then
printf ","
fi
done
...
for (( i=1; i<=$rep; i++ )); do
./laplacian $width $height $depth $iterations $modeGPUTile $gpuBlocks $nThreads 15250 $dt
if [ $i -lt $rep ]; then
printf ","
fi
done
...
printf "tilingTimeX2 = {}\n"
for dh in 15250 10000 7500 5000 2500 1000 500 250; do
...
for (( i=1; i<=$rep; i++ )); do
./laplacian $width $height $depth $iterations $modeGPUTile $gpuBlocks $nThreads $dh 100
if [ $i -lt $rep ]; then
printf ","
fi
done
